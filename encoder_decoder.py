...
class TGS(nn.Module):
    ...
    def forward(self, data):
        node_features = data.dataset.x
        timestamps = data.t
        edge_features = data.edge_index
        node_raw_features = torch.from_numpy(node_features.astype(np.float32)).to("cuda")
        edge_raw_features = torch.from_numpy(edge_features.astype(np.float32)).to("cuda")
        n_node_features = node_raw_features.shape[1]
        n_nodes = node_raw_features.shape[0]
        n_edge_features = edge_raw_features.shape[1]
        time_encoder = TimeEncode(n_node_features)
        embedding_dimension = n_node_features
        node_embedding = self.embedding_module(module_type="graph_sum", 
                                               ...
                                               use_memory=True).compute_embedding(memory=None,
