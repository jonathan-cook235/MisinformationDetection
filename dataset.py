    ...
    def create_dataset(self, dataset_type="dynamic_graph", standardize_features=True, on_gpu=False, oversampling_ratio=1):
        ...
        for tree_file_name in trees_to_parse:
            ...
            if (not self.only_binary) or (label in ['false', 'true']):
                node_features, timestamps, edges = self.build_tree(tree_file_name, tweet_fts=preprocessed_tweet_fts,
                                                       user_fts=preprocessed_user_fts)
                trees.append((news_id, label, node_features, timestamps, edges))

        self.oversample(trees, ids_to_dataset, ratio=oversampling_ratio)

        for news_id, label, node_features, timestamps, edges in trees:
            ...
            if dataset_type == "graph":
                ...
                data_point = torch_geometric.data.Data(x=x, y=y, t=t, edge_index=edge_index)
                if on_gpu:
                    data_point.to(torch.device("cuda"))
                dataset[ids_to_dataset[news_id]].append(data_point)
