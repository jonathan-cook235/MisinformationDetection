...
def get_neighbor_finder(data, uniform=False):
    ...
    yyy = [xx[:,0] for xx in xxx]
    max_node_idx = max([max(yy) for yy in yyy])
    ...
    for source, destination, edge_idx, timestamp in zip(data.sources, data.destinations,
      ...
      for source, destination in zip(source, destination):
        adj_list[source[0]].append(destination)
        adj_list[destination[0]].append(source)
...
class NeighborFinder(object):
  ...
  def find_before(self, src_idx, cut_time):
    ...
    if src_idx < len(self.node_to_edge_timestamps):
      ...
      aaa = self.node_to_neighbors[src_idx][:i] #node index
      bbb = self.node_to_edge_idxs[src_idx][:i] #edge index
  ...
  def get_temporal_neighbor(self, source_nodes, timestamps, n_neighbors):
    ...
    for i, (source_node, timestamp) in enumerate(zip(source_nodes, timestamps)):
      source_neighbors, source_edge_idxs, source_edge_times = self.find_before(i, timestamp)
      # extracts all neighbors, interactions indexes and timestamps of all interactions
      # of user source_node happening before cut_time
